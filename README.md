...
| No manual escape hatch or read-only status check when an operator (not the breaker) recognizes a stuck agent | Four slash commands: `/sit` runs the Poison Arrow protocol on demand, `/koan` presents a Zen koan to break dualistic fixation (`/koan compose <title> | <case>` writes one for the situation at hand, `/koan respond <text>` records which trap the answer fell into, `/koan journal` shows the traps this session keeps returning to — the journal is per session, in memory, and holds no verdict), `/status` reports the session being's state read-only (never settles a pending rebirth, never writes), `/rebirth` forces a fresh being for the session | `src/commands.ts` |
