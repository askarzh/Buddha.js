...
**Goal:** Write a step-by-step markdown tutorial (`docs/interactive-tutorial.md`) that guides the reader through building a Mind from scratch using the Buddha.js library.

**Architecture:** Single markdown document with 5 progressive steps. Each step introduces one concept with explanation and TypeScript code blocks. Bottom-up build: MentalFactor -> Mind -> Being.meditate(). No library code changes — documentation only.

**Tech Stack:** Markdown, TypeScript code examples (illustrative, not executable test files)

---

### Task 1: Write the Interactive Tutorial document

This is the main task. The document has an intro and 5 steps.

**Files:**
- Create: `docs/interactive-tutorial.md`

**Step 1: Create `docs/interactive-tutorial.md`**

Write the full document with this content:
...
In the Mind class, 13 factors are pre-configured across these three categories:
...
The Mind comes pre-loaded with all 13 factors. You can access any of them:
...
The `Mind` class models 13 factors with a simple activation API. For the full Abhidhamma model, explore `Citta`:
...
**Step 2: Verify the document was written correctly**
...
```bash
git add docs/interactive-tutorial.md
git commit -m "docs: add Interactive Tutorial — Building a Mind from Scratch"
```
...
**Step 1: Add the tutorial to `docs/README.md`**

After the Metaphor Guide entry (the last document listing before "Philosophical Foundation"), add:
...
To:
```
- [x] **Interactive Tutorial**: A step-by-step code tutorial building a "Mind" from scratch.
```
...
git commit -m "docs: add Interactive Tutorial to docs index and mark TODO complete"
```
